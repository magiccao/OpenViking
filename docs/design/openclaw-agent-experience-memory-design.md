...
4. 同一轮也保留原有长期记忆 auto recall，但把 experience 和长期记忆分成不同 section。
5. 用统一的 `<openviking-context>` 外壳注入到 latest user message 前。
...
- `buildAutoRecallContext()` 过滤掉 experience memory，避免 experience 出现在长期记忆区。

经验召回复用 `AUTO_RECALL_TIMEOUT_MS = 5_000`，不单独开 experience timeout。
...
然后把两路结果合成一个 `<openviking-context>` block：
...
如果两个 section 都为空，直接 passthrough，不改写 messages。
...
<openviking-agent-experiences ...>...</openviking-agent-experiences>
```
...
这样 `afterTurn()` 写 session 前会清理新旧注入块，避免本轮召回内容进入下一轮抽取。
...
- `cfg.autoRecall` 或 `cfg.agentExperience.enabled && cfg.agentExperience.autoRecall` 至少一个开启。
- latest user message 没有已经包含 `<openviking-context>` 或 legacy `<openviking-agent-experiences>`。
...
经验召回首先受总开关控制：`agentExperience.enabled` 默认关闭。只有显式开启后，才继续看 `agentExperience.autoRecall` 和 gate。

默认 `gatedAutoRecall=true`，会先走 `shouldRecallAgentExperience()`。

硬跳过：

- session bypass。
- query 为空或短于 `agentExperience.minQueryChars`。
- latest user text 已包含 `<openviking-context>` 或 `<openviking-agent-experiences>`。
...
如果 `gatedAutoRecall=false`，跳过上述 task gate，但仍要求 `agentExperience.enabled && agentExperience.autoRecall`。这个选项用于调试或强召回场景，不会绕过总开关。
...
- 5 秒 timeout，失败只 warn，不阻塞 OpenClaw。
...
原有 auto recall 保留，但输出不再直接包 `<relevant-memories>`。它现在生成 `## Long-term Memories` section，再交给统一外壳。
...
- 兼容旧 `<relevant-memories>` 的清理，但新路径不再生成这个外壳。
...
1. assemble 前检查 latest user message 是否已有 `<openviking-context>` 或 legacy `<openviking-agent-experiences>`，有则不重复注入。
2. `sanitizeUserTextForCapture()` 会剥离：
   - `<openviking-context>`
   - `<openviking-agent-experiences>`
   - `<relevant-memories>`
...
- `gatedAutoRecall=false` 时跳过 task gate。
- experience memory 不会出现在 `Long-term Memories` section。
- legacy `<openviking-agent-experiences>` 会阻止重复注入。
- `<openviking-context>` / `<relevant-memories>` / legacy experience block 会被清理。
