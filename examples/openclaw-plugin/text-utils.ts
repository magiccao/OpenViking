...
const OPENVIKING_AGENT_EXPERIENCES_BLOCK_RE =
  /<openviking-agent-experiences\b[^>]*>[\s\S]*?<\/openviking-agent-experiences>/gi;
...
export function sanitizeUserTextForCapture(text: string): string {
  ...
  return text
    ...
    .replace(OPENVIKING_AGENT_EXPERIENCES_BLOCK_RE, " ")
